...
class captcha {
	...
	for ($i = 0; $i < strlen($this->CaptchaString); $i++) {
	    imagettftext($image, $this->charsizemin, mt_rand(0, 15), $i * 25 + 10, mt_rand($this->charsizemin, $imageheight), //$imagelength $imageheight
		$stringcolor, $this->getRandFont(), $this->CaptchaString{$i});
	}
	if ($this->noiseup) {
	    $filter->noise($image, 4);
	}
	if ($this->cryptgaussianblur) {
	    $filter->blur($image, 2);
	}
	if ($this->cryptgrayscal) {
	    imagefilter($image, IMG_FILTER_GRAYSCALE);
	}
	switch (strtoupper($this->cryptsecure)) {
	    case "MD5" : $_SESSION['cryptcode'] = md5($this->CaptchaString);
		break;
	    case "SHA1" : $_SESSION['cryptcode'] = sha1($this->CaptchaString);
		break;
	    default : $_SESSION['cryptcode'] = $this->CaptchaString;
		break;
	}
